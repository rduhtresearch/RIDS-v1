...
run_ca_tests <- function() {
  ...
  .expect("destination_entity is blank for custom rows", is.na(out$destination_entity[1]))
  .expect("cost_code = entered custom code",    out$cost_code[1] == "RDH-FIN-001")
  ...
  .expect("baseline destination_entity is blank", all(is.na(out2$destination_entity)))
  .expect("baseline cost_code values match input", all(out2$cost_code == baseline_rows$cost_centre))
